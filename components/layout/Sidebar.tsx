...
export function Sidebar() {
  ...
  return (
    <aside className="fixed top-0 left-0 bottom-0 w-[244px] flex flex-col z-40"
      style={{ background: "linear-gradient(180deg, #100f17 0%, #0c0b12 100%)", borderRight: "1px solid var(--line)" }}>
      {/* Brand */}
          ...
          return (
            <Link key={item.href} href={item.href}
              className={cn(
                ...
                active
                  ? "text-[var(--cream)]"
                  : "text-[var(--text-2)] hover:bg-[rgba(255,255,255,0.035)] hover:text-[var(--text)]"
              )}
              style={active ? {
        ...
        <div className="bg-[var(--surface-2)] border border-[var(--line)] rounded-xl p-3">
          <div className="flex items-center gap-2.5">
            ...
            <button onClick={() => signOut({ callbackUrl: "/login" })}
              className="p-1.5 bg-transparent border-none cursor-pointer text-[var(--text-3)] hover:text-[var(--text)] rounded-md hover:bg-[rgba(255,255,255,0.05)] transition-colors"
              title="Sign out">
          ...
          <div className="mt-2.5 rounded-[10px] p-2.5"
            ...
            <div className="h-[5px] rounded-full mt-2.5 mb-1 overflow-hidden" style={{ background: "rgba(255,255,255,0.07)" }}>
